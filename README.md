...
- Create posts with title, category, tags, assigned writer, spotlight toggle, and auto-stamped publish date.
...
- `POST /api/posts` — create post `{ title, categoryId, writerId, tags?: string[] | comma string, spotlight?: boolean }`; `publishedAt` auto-filled server-side.
